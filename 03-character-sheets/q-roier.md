# CH-002 — Q!Roier

## Basic identity

- **Character ID:** CH-002
- **Full name / aliases:** Roier (Q!Roier)
- **Played/voiced by:** Not recorded
- **Species/role/archetype:** Not yet established
- **Age:** Not yet established
- **Canon status:** Mixed — rests on one source ([EV-0001](../01-canon-timeline/ev-0001-cruise-ship-first-meeting.md)).

## Visual design

Not yet established.

## Personality

- Social connector — recognizes people and actively brings them together
  (drags Foolish over to meet Juan). [CANON FACT]
- Warm/casual manner in his one line of dialogue so far. [CANON FACT]

## Speech patterns

- Speaks Spanish as demonstrated ("Este es mi amigo, Juan.") — too little
  sample yet to characterize further. [CANON FACT]

## Languages

- Spanish (native or fluent — [STRONG INFERENCE], based on ease of
  speech vs. Foolish's learner-level Spanish).

## Recurring phrases

None established yet.

## Motivations

Not yet established directly.

## Fears / insecurities

Not yet established.

## Habits

Not yet established.

## Competencies

Not yet established.

## Flaws

Not yet established.

## Faction affiliations

None yet.

## Important possessions

None established yet.

## Current status

- **As of:** EV-0001
- Aboard the cruise ship; reunites with Foolish and reintroduces him to
  Juan. [CANON FACT]

## Chronological character arc

| Event ID | What happens for this character | Tag |
|---|---|---|
| EV-0001 | Reconnects with Foolish; brings Foolish and Juan together | [CANON FACT] |

## Relationships

- [Q!Foolish & Q!Roier](../04-relationship-sheets/q-foolish-and-q-roier.md)
- [Q!Juan & Q!Roier](../04-relationship-sheets/q-juan-and-q-roier.md)

## Secrets

| Secret | Hidden from | Known to | Revealed? (Event ID or "not yet") | Tag |
|---|---|---|---|---|
| Extent of memory loss re: last island | Not stated as hidden on purpose — simply not raised in EV-0001 | Roier (presumably, to the extent he's aware of his own gaps) | Not yet | [MY INTERPRETATION] |

## What this character knows at different points in the timeline

| Event ID / date | Revelation (REV-ID) | Source of knowledge | Tag |
|---|---|---|---|
| EV-0001 | REV-0002 (his own memory loss, to the extent he's aware of it) | Firsthand | [MY INTERPRETATION] |

## What the audience knows that they do not

| Event ID / date | Revelation (REV-ID) | Character remains unaware until | Tag |
|---|---|---|---|
| EV-0001 | REV-0001 (full extent of Foolish's island/purgatory history — unclear if Roier remembers this, given his memory loss) | Unresolved | [MY INTERPRETATION] |
| EV-0001 | REV-0003 (Juan's prison history) | Unresolved | [MY INTERPRETATION] |
| EV-0001 | REV-0004 (Foolish's private resolve) | Unresolved | [MY INTERPRETATION] |

## Canon evidence

- [EV-0001](../01-canon-timeline/ev-0001-cruise-ship-first-meeting.md) / [transcript](../02-transcripts/raw/undated-cruise-ship-foolish-roier-juan-first-meeting.md)

## My interpretation

Roier's memory loss (REV-0002) is entirely author narration, not shown in
dialogue — treat it as intended backstory/setup rather than confirmed
in-scene fact. It creates a specific asymmetry worth preserving in future
writing: Foolish may remember their shared history in detail (including
feeling paternal toward Roier) while Roier may not remember it clearly at
all. That gap is currently unresolved and unacknowledged by either
character on-screen.

## Writing notes

- Voice checklist: too little dialogue sample yet — keep an eye out for
  more before locking in a voice.
- Things to avoid: don't have Roier reference specific island memories
  confidently unless/until his memory loss is clarified — err toward
  vagueness or avoidance rather than sudden recall.
- Open craft questions: how much does Roier actually remember, and does
  he know he's forgetting things, or is it invisible to him?
